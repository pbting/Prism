...
import com.road.yishi.log.mina.ServerPackage;

public class ClassUtil {

	public static void main(String[] args) {
		ArrayList<Class> impInterfaces = getAllClassByInterface(ServerPackage.class);
		for(Class clazz : impInterfaces){
		...
		if (clazz.isInterface()) {
			try {
				...
				for (int i = 0; i < allClass.size(); i++) {
					/**
					 * 判断是不是同一个接口 该方法的解析，请参考博客： http://blog.csdn.net/u010156024/article/details/44875195
					 */
	...
	private static ArrayList<Class> getAllClass(String packagename) {
		...
		try {
			...
			/**
			 * 这里面的路径使用的是相对路径 如果大家在测试的时候获取不到，请理清目前工程所在的路径 使用相对路径更加稳定！ 另外，路径中切不可包含空格、特殊字符等！ 本人在测试过程中由于空格，吃了大亏！！！
			 */
